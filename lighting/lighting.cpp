/***************************************************
显式含有光照的off模型：加入了每个顶点的法向量的计算，法向量计算应该没有什么问题
效果还可以
************************************************/
...
{
	Init();
	Shader ourShader("lightingshader .vs", "lightingshader .fs");
	//read_off("D:/MyCode/VoxelTest/chairs_shape/2.off");
	read_off("12.off");
	calNormal();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 transform;
		transform = glm::scale(transform, glm::vec3(1.f /2.f/ (max_y - min_y)));
...
void calNormal()
{
	...
	for (idx = 0; idx < nFaces * 3; idx+=3){
		...
		NormalBuffer[index2 * 3 + 2] = normal.z; //也可以使用 +=，比较不同的效果
	}
...
void grab(string img_name)
{
	...
	std::cout << "load image success" << endl;
...
void img_proccess(string img_name)
{
	...
	cout << "opencv write img success" << endl;
}
